...
def pif(value):
    """
    pif converts ascii to (int|float)
    """
    ...
    def _extinf(self):
        ...
        if "#EXTINF" in self.tags:
            if isinstance(self.tags["#EXTINF"], str):
                self.tags["#EXTINF"] = float(self.tags["#EXTINF"].rsplit(",", 1)[0])
            self.duration=self.tags["#EXTINF"]
...
##                del segment.tmp
    ...
    def _endlist_chk(self, line):
        ...
        if "#EXT-X-ENDLIST" in line:
            ...
            print(out)
            self.write_m3u8()
            with open(out, "a", encoding="utf8") as npm3u8:
                npm3u8.write(f'#EXT-X-ENDLIST\n')
            sys.exit()
    ...
    def write_m3u8(self):
        ...
        with open(out, "w", encoding="utf8") as npm3u8:
            ...
            for segment in self.segments:
                ...
                _ = [npm3u8.write(j + "\n") for j in stanza]
        throttle = self.segments[-1].duration * 0.7
        time.sleep(throttle)
...
##                splice_cue = s[1]
##                if splice_pts:
##                    print(segment.start,splice_pts, segment.end)
...
##                        self._chk_cue_time()
    ...
    def _chk_sidecar_cues2(self, segment):
        """
        _chk_sidecar_cues checks the insert pts time
        for the next sidecar cue and inserts the cue if needed.
        """
        ...
        if self.sidecar:
            for s in list(self.sidecar):
                ...
                if segment.start:
                  half = segment.duration/2
                if splice_pts:
                    if (segment.start) <= splice_pts <= (segment.start + segment.duration):
                        self.sidecar.remove(s)
                        self.scte35.cue = Cue(splice_cue)
...
##        base_uri=based[0]+"/"
##    print('BASE', base_uri)
##    with reader(m3u8) as inm3u8:            
##        lines =inm3u8.read().decode().split('\n')
##        for line in lines:
##            if line.startswith("#EXTINF"):
##                seg =Segment(lines,m3u8,0,base_uri,first)
##                seg.decode()
##                playlists.append(m3u8)
##                return playlists
##            chunky.append(line)
##            #if line.startswith("#EXT-X-STREAM-INF"):
##             #   playlist.append(inm3u8.readline())
##            if line and  not line.startswith("#"):
##                print("HERE",base_uri,line)
##                seg = Segment(chunky,base_uri+line,None,base_uri, first)
##                seg.decode()
##                print("HEY",seg)
##                chunk=[]
##                playlists.append(seg)
##                if first:
##                    first=False
####            if line.startswith("#EXT-X-MEDIA:TYPE"):
...
def cli():
    ...
    Two lines of code gives you a full command line tool.
